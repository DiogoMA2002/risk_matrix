  ...
  <div class="bg-white bg-opacity-90 backdrop-blur-sm rounded-xl shadow-md p-6 mb-6">
    <header class="mb-6">
      ...
      <div class="flex justify-between items-center mt-4">
        ...
        <button
          ...
          class="ml-4 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
          :disabled="loading"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          >
            <path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M12 4v12m0 0l-3-3m3 3l3-3m-7 5h8a2 2 0 002-2V6a2 2 0 00-2-2h-3"
            />
          </svg>
          {{ labels.import }}
        </button>
    ...
    >
      <div class="bg-white bg-opacity-90 backdrop-blur-sm rounded-xl shadow-xl p-6 w-full max-w-md">
        <h3 class="text-xl font-semibold text-blue-800 mb-4">{{ labels.importTitle }}</h3>
        <input
          type="file"
...
export default {
  ...
  computed: {
    ...
    labels() {
      return {
        ...
        importInvalidStructure: "Estrutura do arquivo JSON inválida. O arquivo deve conter 'title' e 'questions'.",
  ...
  methods: {
    ...
    async handleFileImport(event) {
      ...
      if (!file) {
        this.showToast(this.labels.importNoFile, 'error');
        return;
      }

      if (!file.name.endsWith('.json')) {
        this.showToast(this.labels.importNotJson, 'error');
        return;
      }
      ...
      reader.onload = (e) => {
        try {
          ...
          if (!jsonData.title || !jsonData.questions) {
            throw new Error(this.labels.importInvalidStructure);
          }
          ...
          this.showToast(`${this.labels.importError}: ${error.message}`, 'error');
        }
      };
      reader.onerror = (error) => {
        console.error('Error reading file:', error);
        this.showToast(this.labels.importError, 'error');
      };
    ...
    async showConfirmDialog(title, message) {
      // Replace with your confirm implementation.
