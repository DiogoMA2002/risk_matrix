...
public class AnswerService {
    ...
    public AnswerDTO submitAnswerByQuestionText(AnswerDTO answerDTO) {
        if (answerDTO.getEmail() == null || answerDTO.getEmail().trim().isEmpty()) {
            throw new IllegalArgumentException("Email is required.");
        }

        // Find question by text instead of ID
        Question question = questionRepository.findByQuestionText(answerDTO.getQuestionText())
                .orElseThrow(() -> new NotFoundException("Question not found for text: " + answerDTO.getQuestionText()));

        // Debug: Log the question and available options
        System.out.println("Question ID: " + question.getId());
        System.out.println("Question Text: " + question.getQuestionText());
        System.out.println("User Response: '" + answerDTO.getUserResponse() + "'");
        ...
        QuestionOption selectedOption = question.getOptions().stream()
                ...
                .orElseThrow(() -> {
                    String availableOptions = question.getOptions().stream()
                            ...
                            .collect(Collectors.joining(", "));
                    return new InvalidOptionException("Invalid response: '" + answerDTO.getUserResponse() + 
                            "'. Available options for question '" + question.getQuestionText() + 
                            "' are: " + availableOptions);
                });
    ...
    public List<AnswerDTO> submitMultipleAnswersByText(List<AnswerDTO> answers) {
        String submissionId = UUID.randomUUID().toString();
        return answers.stream()
                .map(ans -> {
                    ans.setSubmissionId(submissionId);
                    return submitAnswerByQuestionText(ans);
                }).toList();
    }
}
